...
if {$cal(hour) in {23 11}} {
	set fl [open aan.out w+]
	close $fl
	puts ...\n0:0
	exit
}
if {$cal(hour) ni {00 01 02 03 04 05 07 12 13 14 15 16 17 18}} {exit}
...
#puts $title
#puts $pageid
#if {$pageid != 6529924} {continue}
			lassign {{} 0} fb ts1bl
			if {$title in {{Benutzer Diskussion:Maimaid} {Wikipedia:Redaktion Geisteswissenschaften}}} {continue}
			if {$title in {{Benutzer:Messina/Diskussionen zu Entwürfen}			{Wikipedia:Redaktion Chemie/Qualitätssicherung}
								 Benutzer:Luke081515/Baustellen							 Benutzer:Malo95/Baustelle
								{Wikipedia Diskussion:Formatvorlage Bahnstrecke}	{Wikipedia Diskussion:Hauptseite/Jahrestage}
								{Diskussion:Vogelgrippe H5N1}								 Diskussion:Parallelwelt								}} {
				incr ts1bl
			}
			switch $title {
				 Benutzer:TaxonBot/AA												{set rvtitle  Wikipedia:Auskunft}
				{Wikipedia:WikiProjekt Begriffsklärungsseiten/Fließband} {set rvtitle {Wikipedia:WikiProjekt Begriffsklärungsseiten/Fließband/Knacknüsse}}
				{Wikipedia:WikiProjekt Begriffsklärungsseiten/Fließband/Knacknüsse} {
																							 set rvtitle {Wikipedia:WikiProjekt Begriffsklärungsseiten/Fließband}
																							 set fb "\n\n\{\{Kasten|1=per Fließband aus den Knacknüssen zurück ~~~~\}\}"
																							}
				{Benutzer Diskussion:Luke081515/Archiv}						{set rvtitle {Benutzer Diskussion:Luke081515}}
				{Benutzer Diskussion:Luke081515Bot/Archiv}					{set rvtitle {Benutzer Diskussion:Luke081515Bot}}
				{Benutzer Diskussion:GS63/B/Archiv/1}							{set rvtitle {Diskussion:Berlin}}
				{Benutzer Diskussion:GS63/B/Archiv/2}							{set rvtitle {Diskussion:Berlin}}
				{Benutzer Diskussion:GS63/B/Archiv/3}							{set rvtitle {Diskussion:Berlin}}
				{Benutzer Diskussion:GS63/B/Archiv/4}							{set rvtitle {Diskussion:Berlin}}
			...
			while {$ec == 1} {if [catch {
				...
				foreach prop $props {
					...
					lassign {0 0} el na
					if {		[string match -nocase {*\{\{erledigt|*} $tx]
							|| [string match -nocase {*\{\{erledigt |*} $tx]} {
						incr el
					}
					if {		[string match -nocase {*\{\{nicht archivieren*} $tx]
							|| [string match -nocase {*\{\{OTRS*} $tx]
						...
						foreach prop $props {
							dict with prop {
								...
								if {($tc >= ${Mindestbeiträge} || ($moderl && $el)) && !$na && !$lock} {
									...
									regsub -nocase -- {\(\(Lemma\)\)|\(\(FULLPAGENAME\)\)} [string trim $Ziel '] [string map [list & \\&] $title] nZiel
				...
				if {$content_orig eq [contents t $title x]} {
					if {$arlist ne {}} {
						...
						foreach artgt [lsort -unique [dict keys $arlist]] {
							unset -nocomplain aranchor
							set aranchor [dict get [lindex [get [post $wiki {*}$format / action parse / text [dict get $arlist $artgt] / prop sections] parse sections] 0] line]
							if [catch {
								if {[dict get {*}[
						...
						set rmsumm "Bot: $complarcount Abschnitt[expr {$complarcount > 1 ? {e} : {}}] nach [join $tgts {, }] archiviert $lastrev"
#						puts $rmlist
						...
						foreach {arti artx} $arlist {
							...
							unset -nocomplain aranchor
							set aranchor [dict get [lindex [get [post $wiki {*}$format / action parse / text $artx / prop sections] parse sections] 0] line]
							set arsumm "Bot: $arct Abschnitt[expr {$arct > 1 ? {e} : {}}] aus \[\[$title\]\] (ab Abschnitt \[\[$arti#$aranchor|$aranchor\]\]) archiviert"
							puts $arti:\n[string length $artx]
							puts [set rml [edit $arti $arsumm {} / appendtext [
								expr {[dict exists [page [post $wiki {*}$get / titles $arti / prop info]] missing] == 1 ? "$Kopfvorlage" : {}}
							]$artx]]
							if {[lindex $rml 0] eq {error}} {
								puts [edid 987926 Autoarchiv-Fehler "\[\[$title\]\] → \[\[$arti\]\]" / section new / minor]
							}
						}
					}
					decr ec
...
#				exit
			...
			puts [edid 63277 {Log: AA} {} / appendtext "\n* '''[clock format [clock seconds] -format %Y-%m-%dT%TZ] AutoArchiv: Fehler in \[\[:w:de:$title\]\]!'''" / minor]
...
puts {end of task}
set lang test ; source langwiki.tcl ; #set token [login $wiki]
puts [edid 63277 {Log: AA} {} / appendtext "\n* '''[clock format [clock seconds] -format %Y-%m-%dT%TZ] AutoArchiv: Task finished!'''" / minor]
