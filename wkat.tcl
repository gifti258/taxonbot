...
mysqlclose $db
puts "[clock format [clock seconds] -format %T]:wKATkat-1 komplett"
foreach {pt cl} $lwKAT {
	foreach item $cl {
...
puts "[clock format [clock seconds] -format %T]:wKATkat-2 komplett"
foreach item $nlwKAT {
	set lcat {}
	...
	set pt [string map {_ { }} [
		mysqlsel $db "
			select page_title
			from page
			where page_id = $item and !page_namespace
		;" -list
	]]
	mysqlclose $db
	lappend wKATkat $pt [lsort -unique [join $lcat]]
}
puts "[clock format [set catsecs [clock seconds]] -format %T]:wKATkat-3 komplett"
...
mysqlclose $db
puts "[clock format [set catsecs [clock seconds]] -format %T]:wKATkat-4 komplett"
