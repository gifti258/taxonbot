...
puts [set offset 20180607020001]

lassign {} olrc rcline logtype logparams
while 1 {
	while 1 {
		if ![catch {
			set lrc [get [post $wiki {*}$query {*}$format / list recentchanges / rcnamespace 0|2 / rcprop timestamp|title|ids|user|comment|loginfo / rcend 20180605215959 / rcstart $offset / rclimit 5000] query recentchanges]
		}] {break}
	}
	set dictoffset [lindex $lrc end]
	puts [set offset [expr [dict with dictoffset {utc ^ $timestamp %Y-%m-%dT%TZ %Y%m%d%H%M%S {}}] - 1]]
	if {$lrc ne $olrc} {
