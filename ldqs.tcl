...
catch {if {[exec pgrep -cxu taxonbot ldqs.tcl] > 1} {exit}}
...
proc lddata {} {
	...
	if {[string first Result1= $lddata] == -1} {
		return {}
	} else {
		return \{\{[join [lappend lddata Bot=TaxonBot] |]\}\}
	}
}
...
while 1 {
	...
	if {[catch {
		set ldpage {}
		set ch Wikipedia:Qualitätssicherung/[string trim [
	...
	if {[exists lastsect] && $sect == $lastsect} {
