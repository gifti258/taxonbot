...
set tdate [clock format [clock scan [lindex [join [
	page [post $wiki {*}$get / titles $tgt / prop revisions / rvprop timestamp / rvstartid $rvtgt / rvlimit 1] revisions
]] 1] -format %Y-%m-%dT%TZ] -format {%e. %B %Y} -timezone :Europe/Berlin -locale de]
...
if [missing $dtgt] {
	puts \n[edit $dtgt Lizenzhinweis $prelicence$licence$endlicence]
} elseif {[string trim [conts t $dtgt 0]] eq {}} {
	puts \n[edit $dtgt Lizenzhinweis $prelicence$licence$endlicence / section 0]
} else {
	puts \n[edit $dtgt Lizenzhinweis {} / appendtext \n\n$prelicence$licence$endlicence / section 0]
}
