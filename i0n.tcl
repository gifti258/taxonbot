...
#puts [incr zzzzz]:$timestamp
	puts "Version [incr zyx2]: $timestamp"
			set xml [string map [list $i "    <revision>\n      <id>$revid</id>\n$i"] $xml]
			if $parentid {set xml [string map [list $i "      <parentid>$parentid</parentid>\n$i"] $xml]}
			set xml [string map [list $i "      <timestamp>$timestamp</timestamp>\n      <contributor>\n$i"] $xml]
			if $userid {
				set xml [string map [list $i "        <username>[
					string map {< &lt; > &gt; & &amp;} $user
				]</username>\n        <id>$userid</id>\n$i"] $xml]
			} else {
				set xml [string map [list $i "        <ip>$user</ip>\n$i"] $xml]
			}
			set xml [string map [list $i "      </contributor>\n$i"] $xml]
			if [dict exists $1 minor] {set xml [string map [list $i "      <minor />\n$i"] $xml]}
			if {$comment ne {}} {set xml [string map [list $i "      <comment>[
				string map {< &lt; > &gt; & &amp;} $comment
			]</comment>\n$i"] $xml]}
			set xml [string map [list $i "      <model>wikitext</model>\n      <format>text/x-wiki</format>\n$i"] $xml]
			set xml [string map [list $i "      <text xml:space=\"preserve\" bytes=\"$size\">[
				string map {< &lt; > &gt; & &amp;} ${*}
			]</text>\n$i"] $xml]
			set xml [string map [list $i "      <sha1>$sha1</sha1>\n    </revision>\n$i"] $xml]
		}
		if {[string bytelength $xml] > 95000000} {
			set xml [string map [list $i\n {}] $xml]
			save_file xml$nr.xml $xml
			set lrev [regexp -all -- {<revision.*?</revision} $xml]
			set fsize [file size xml$nr.xml]
			lappend lxml $nr $fsize $lrev
			incr nr
			set xml $xml0
		}
		if {$revid == $offset} {
...
foreach {1 2 3} [rev $hist $offset] {
	puts "\nFile Size (xml$1): $2"
	puts "Liste $1: $3 Versionen"
}
puts {}
exit
