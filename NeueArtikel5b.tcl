...
foreach key {wALTkat wGEOkat wVFkat wDFkat wBWkat} {
...
set bwwkat [read [set f [open BWWORKLIST/@bwwkat.db r]]] ; close $f
foreach key {bwwKALPkat} {
	set $key [dict get $bwwkat $key]
}
...
proc computerspiele {portal nportal data} {
	...
	dict with data {
		...
		set lncportal [lsort -stride 2 -integer -decreasing $lncportal]
		foreach {-- ncportal} $lncportal {
			lappend nlncportal $ncportal
		}
		set nacount [regexp -all -- {\[\[:} $lncportal]
		return [string map {& {\&}} [join $nlncportal ,\n]]
	}
}
...
#puts $rvpage:$rvlink ; gets stdin
					if {$rvpage eq {Schreibwettbewerb}} {
						lappend rvblock [format {[[:%s]]<small> [[WP:SW|(SW)]]</small>} $rv]
							...
							if [matchtemplate $rv Vorlage:Schreibwettbewerb] {
								set swplus { ([[WP:SW|SW]]+}
							} else {
								set swplus { (}
							}
						lappend rvblock [format {[[:%s]]<small>%s[[WP:RV%s#%s|%s]])</small>} $rv $swplus $rvlink $rv $rvlink]
...
proc w portal {
	...
	global wGEOkat wVFkat wDFkat wDFit wBWkat hidden wcount listformat
...
#puts wGEOmatch:$wGEOmatch
		...
		if {({ALL} in $LISTS && {-BW} ni $LISTS) || {BW} in $LISTS} {
			set wBWblock {}
			set wBWmatch [catselect wBW -- $catdb $icatdb]
#puts wBWmatch:$wBWmatch
			if ![empty wBWmatch] {
				foreach wBW [lsort -unique $wBWmatch] {lappend orpt '[sql <- $wBW]'}
				set db [get_db dewiki]
				mysqlreceive $db "
					select page_title
					from page, categorylinks
					where cl_from = page_id and page_namespace = 0 and page_title in ([join $orpt ,]) and (cl_to like 'Wikipedia:Bilderwunsch%' or cl_to = 'Wikipedia:Mineral_ohne_Bild')
					order by page_title
				;" pt {
					lappend wBWblock "\[\[:[sql -> $pt]\]\]"
				}
				mysqlclose $db
				set wBWblock [lsort -unique $wBWblock]
				unset -nocomplain orpt
			}
			incr wcount [set wBWcount [llength $wBWblock]]
			set wBWblock [join $wBWblock "\n[expr {$listformat eq {SHORTLIST} ? {·} : {*}}] "]
			set wBWbranch ";\[\[Datei:Photo-request.svg|30x15px|text-unten|Bilderwunsch|link=:Kategorie:Wikipedia:Bilderwunsch\]\]&nbsp;Bilderwunsch<small> ([tdot $wBWcount])</small>"
			if ![empty wBWblock] {
				append WORKLIST \n$wBWbranch\n[expr {$listformat eq {SHORTLIST} ? {} : $listformat eq {CLIST} ? {# } : {* }}]$wBWblock
			} elseif {({ALL} in $ALWAYSSHOW && {-BW} ni $ALWAYSSHOW) || {BW} in $ALWAYSSHOW} {
				append WORKLIST \n$wBWbranch\n$EMPTY
			}
		}
...
#puts wDWmatch:$wDWmatch
			if ![empty wDWmatch] {
				...
				mysqlreceive $db "
					select page_title
					from page, categorylinks
					where cl_from = page_id and page_namespace in (0, 1) and page_title in ([join $orpt {, }]) and cl_to like 'Wikipedia:Defekte_Weblinks%'
					order by page_title
				;" pt {
					set pt [sql -> $pt]
...
proc bw portal {
	global db bwwKALPkat
	global bwwcount bwwkandcount bwwlwkandcount bwwlwabwcount bwwreviewcount listformat
	lassign {0 LIST ALL {} {''Zurzeit keine''} {}} bwwcount listformat LISTS ALWAYSSHOW EMPTY BWWORKLIST
	set bwwdb [read [set f [open BWWORKLIST/[string map {{ } _ ~ ~~~~~ / ~ ! ´´´´} $portal] r]]] ; close $f
	dict with bwwdb {
		set listformat [dict get $param listformat]
		set LISTS [join [split [dict get $param LISTS] ,]]
		set ALWAYSSHOW [join [split [dict get $param ALWAYSSHOW] ,]]
		set EMPTY [dict get $param EMPTY]
		if [catch {set KALP [dict get $param KALP]}] {set KALP {}}
#		puts $KALP
#set LISTS ALL
#set ALWAYSSHOW ALL
		if {({ALL} in $LISTS && {-KALP} ni $LISTS) || {KALP} in $LISTS} {
			lassign {} bwwKALPkandblock bwwKALPlwabwblock bwwKALPlwkandblock
			set bwwKALPmatch [catselect bwwKALP -- $catdb $icatdb]
#puts bwwKALPmatch:$bwwKALPmatch
			if ![empty bwwKALPmatch] {
				foreach bwwKALP [lsort -unique $bwwKALPmatch] {lappend orpt '[sql <- $bwwKALP]'}
				set db [get_db dewiki]
				mysqlreceive $db "
					select page_title, tl_title
					from page join templatelinks on tl_from = page_id
					where page_title in ([join $orpt ,]) and !page_namespace and !tl_from_namespace and tl_namespace = 10 and (tl_title = 'Kandidat' or tl_title like 'Lesenswert-%')
					order by page_title
				;" {pt tlt} {
					if {$KALP eq {UNION}} {
						lappend bwwKALPkandblock "\[\[:[sql -> $pt]\]\]"
					} else {
						switch $tlt {
							Kandidat						{lappend bwwKALPkandblock "\[\[:[sql -> $pt]\]\]"}
							Lesenswert-Abwahl			{lappend bwwKALPlwabwblock "\[\[:[sql -> $pt]\]\]"}
							Lesenswert-Kandidat		{lappend bwwKALPlwkandblock "\[\[:[sql -> $pt]\]\]"}
							Lesenswert-Wiederwahl	{lappend bwwKALPkandblock "\[\[:[sql -> $pt]\]\]"}
						}
					}
				}
				mysqlclose $db
			...
			if {$KALP eq {UNION}} {
				incr bwwcount [set bwwkandcount [llength $bwwKALPkandblock]]
			} else {
				incr bwwcount [set bwwkandcount [llength $bwwKALPkandblock]]
				incr bwwcount [set bwwlwabwcount [llength $bwwKALPlwabwblock]]
				incr bwwcount [set bwwlwkandcount [llength $bwwKALPlwkandblock]]
			}
			if {$KALP eq {UNION}} {
				set bwwKALPkandblock [join $bwwKALPkandblock "\n[expr {$listformat eq {SHORTLIST} ? {·} : {*}}] "]
			} else {
				set bwwKALPkandblock [join $bwwKALPkandblock "\n[expr {$listformat eq {SHORTLIST} ? {·} : {*}}] "]
				set bwwKALPlwabwblock [join $bwwKALPlwabwblock "\n[expr {$listformat eq {SHORTLIST} ? {·} : {*}}] "]
				set bwwKALPlwkandblock [join $bwwKALPlwkandblock "\n[expr {$listformat eq {SHORTLIST} ? {·} : {*}}] "]
			}
			if {$KALP eq {UNION}} {
				set bwwKALPkandbranch ";\[\[Datei:QSicon Kand.svg|30x15px|text-unten|Auszeichnungskandidatur|link=:Kategorie:Wikipedia:Kandidatur von Artikeln, Listen und Portalen\]\]&nbsp;Auszeichnungskandidatur<small> ([tdot $bwwkandcount])</small>"
			} else {
				set bwwKALPkandbranch ";\[\[Datei:QSicon Kand.svg|30x15px|text-unten|Auszeichnungskandidatur|link=:Kategorie:Wikipedia:Kandidatur von Artikeln, Listen und Portalen\]\]&nbsp;Auszeichnungskandidatur<small> ([tdot $bwwkandcount])</small>"
				set bwwKALPlwabwbranch ";\[\[Datei:Qsicon lesenswert Abwahl.svg|30x15px|text-unten|Abwahl Lesenswert|link=:Wikipedia:Kandidaten für lesenswerte Artikel\]\]&nbsp;Abwahl Lesenswert<small> ([tdot $bwwlwabwcount])</small>"
				set bwwKALPlwkandbranch ";\[\[Datei:Qsicon lesenswert Kandidat.svg|30x15px|text-unten|Kandidat Lesenswert|link=:Wikipedia:Kandidaten für lesenswerte Artikel\]\]&nbsp;Kandidat Lesenswert<small> ([tdot $bwwlwkandcount])</small>"
			}
			if {$KALP eq {UNION}} {
				if ![empty bwwKALPkandblock] {
					append BWWORKLIST \n$bwwKALPkandbranch\n[expr {$listformat eq {SHORTLIST} ? {} : $listformat eq {CLIST} ? {# } : {* }}]$bwwKALPkandblock
				} elseif {({ALL} in $ALWAYSSHOW && {-KALP} ni $ALWAYSSHOW) || {KALP} in $ALWAYSSHOW} {
					append BWWORKLIST \n$bwwKALPkandbranch\n$EMPTY
				}
			} else {
				if ![empty bwwKALPkandblock] {
				...
				if ![empty bwwKALPlwkandblock] {
					append BWWORKLIST \n$bwwKALPlwkandbranch\n[expr {$listformat eq {SHORTLIST} ? {} : $listformat eq {CLIST} ? {# } : {* }}]$bwwKALPlwkandblock
				} elseif {({ALL} in $ALWAYSSHOW && {-KALP} ni $ALWAYSSHOW) || {KALP} in $ALWAYSSHOW} {
					append BWWORKLIST \n$bwwKALPlwkandbranch\n$EMPTY
				}
				if ![empty bwwKALPlwabwblock] {
					append BWWORKLIST \n$bwwKALPlwabwbranch\n[expr {$listformat eq {SHORTLIST} ? {} : $listformat eq {CLIST} ? {# } : {* }}]$bwwKALPlwabwblock
				} elseif {({ALL} in $ALWAYSSHOW && {-KALP} ni $ALWAYSSHOW) || {KALP} in $ALWAYSSHOW} {
					append BWWORKLIST \n$bwwKALPlwabwbranch\n$EMPTY
				}
			}
		...
		if {{RV} in $LISTS} {
			set bwwreviewblock {}
			set bwwreviewmatch [catselect bwwKALP -- $catdb $icatdb]
#puts bwwreviewmatch:$bwwreviewmatch
			if ![empty bwwreviewmatch] {
				foreach bwwreview [lsort -unique $bwwreviewmatch] {
					if {[matchtemplate $bwwreview Vorlage:Review] || [matchtemplate $bwwreview Vorlage:Schreibwettbewerb] || [matchtemplate $bwwreview Vorlage:UIBK-Bio]} {
						regexp -- {\{\{(Review\|(.*?)|Schreibwettbewerb|UIBK-Bio)??\}\}} [conts t $bwwreview x] -- bwwreviewpage bwwreviewlink
#puts $bwwreviewpage:$bwwreviewlink ; gets stdin
						if {$bwwreviewpage eq {Schreibwettbewerb}} {
							lappend bwwreviewblock [format {[[:%s]]<small> [[WP:SW|(SW)]]</small>} $bwwreview]
						} elseif {[string first Review $bwwreviewpage] > -1} {
							if [matchtemplate $bwwreview Vorlage:Schreibwettbewerb] {
								set swplus { ([[WP:SW|SW]]+}
							} else {
								set swplus { (}
							}
							lappend bwwreviewblock [format {[[:%s]]<small>%s[[WP:RV%s#%s|%s]])</small>} $bwwreview $swplus $bwwreviewlink $bwwreview $bwwreviewlink]
						} elseif {$bwwreviewpage eq {UIBK-Bio}} {
							lappend bwwreviewblock [format {[[:%s]]<small> [[WD:WikiProjekt UIBK Biologie#%s|(UIBK-Bio)]]</small>} $bwwreview $bwwreview]
						}
					}
				}
			}
			incr bwwcount [set bwwreviewcount [llength $bwwreviewblock]]
			set bwwreviewblock [join $bwwreviewblock "\n[expr {$listformat eq {SHORTLIST} ? {·} : {*}}] "]
			set bwwreviewbranch ";\[\[Datei:Qsicon inArbeit blue.svg|30x15px|text-unten|Review|link=:Kategorie:Wikipedia:Reviewprozess\]\]&nbsp;Review<small> ([tdot $bwwreviewcount])</small>"
			if ![empty bwwreviewblock] {
				append BWWORKLIST \n$bwwreviewbranch\n[expr {$listformat eq {SHORTLIST} ? {} : $listformat eq {CLIST} ? {# } : {* }}]$bwwreviewblock
			} elseif {{RV} in $ALWAYSSHOW} {
				append BWWORKLIST \n$bwwreviewbranch\n$EMPTY
			}
		}
		return [string map {& {\&}} $BWWORKLIST]
	}
}
...
#puts [edit user:TaxonBota/qstest1 test [string map {{\&} &} $WORKLIST]] ; exit
			...
			if {$portal in $lbwportal} {
				set br 4
				set BWWORKLIST [bw $portal]
				if {$listformat ne {TABLE}} {
					set nportal [string map {{\&} &} [regsub -- {(<!--MB-BWWORKLIST-->).*<!--MB-BWWORKLIST-->} $nportal \\1$BWWORKLIST\n\\1]]
					if ![empty summary] {
						append summary "; BWWORKLIST: [tdot $bwwcount]"
					} else {
						set summary "Bot: BWWORKLIST: [tdot $bwwcount]"
					}
				}
			}
