...
catch {if {[exec pgrep -cxu taxonbot NeueArtikel3vb.] > 1} {exit}}
...
set nadb [read [set f [open NeueArtikel.match/NeueArtikel-$tday-b r]]] ; close $f
...
#	if {$portal ne {Portal:Hannover/Neue Artikel}} {continue} else {set 1x 1}
#	if {$portal ne {Benutzer:Wartungsstube/Musik}} {continue} else {set 1x 1}
	while 1 {
		try {
			if {[exec pgrep -cxu taxonbot NeueArtikel5.tc] < 10} {
				if [catch {exec ./NeueArtikel5.tcl "[list $portal $data]" &}] {
					puts "Fehler beim Aufrufen von NeueArtikel5 mit $portal: $data"
				}
				after 6000
				break
			}
