...
set tsto 20171219230000

set db [get_db dewiki]
mysqlreceive $db "
	select rc_timestamp, rc_namespace, rc_title, rc_cur_id, rc_source, rc_log_type, rc_params
	from recentchanges
	where rc_timestamp < $tsto and ((!rc_namespace and rc_source not in ('mw.edit','wb')) or rc_log_type in ('merge','move'))
	order by rc_timestamp
;" {rcts rcns rct rccid rcsrc rclt rcparam} {
#	if ![exists tsfrom] {set tsfrom $rcts}
#	lappend lt $rct
	set rctgt [set rct [sql -> $rct]]
puts $rcsrc
	if {$rcsrc eq {}} {
		set rclt new
	} elseif {$rclt in {merge move}} {
		set tgt [lindex [split $rcparam \"] 3]
		if ![nstodns [lindex [split $tgt :] 0]] {
			set rctgt $tgt
		} else {
			continue
		}
#	} elseif {$rclt eq {import}} {
#		if $rcns {continue}
	}
	lappend lt $rctgt
	if {$rct in $lt} {
		lappend lna [list $rcts $rcns $rccid $rclt $rct $rctgt]
	} else {
		lappend lna [list $rcts $rcns $rccid ---$rclt $rct $rctgt]
	}
}
...
set db [get_db dewiki]
mysqlreceive $db "
	select log_timestamp, log_type, log_namespace, log_title, log_params
	from logging
	where log_type = 'import' and log_timestamp >= 20171218000000 and log_timestamp < $tsto and log_namespace = 2
	order by log_timestamp
;" {lts ltype lns lt lparams} {
	puts [list $lts $lns $ltype $lt $lparams]
}
exit
	lappend l2 [list $lts $lns $ltype $lt $lparams]
	puts \n[list $lts $lns $ltype $lt]\n$lparams
	if {$ltype eq {move}} {
		set tgt [lindex [split $lparams \"] 3]
		if ![nstodns [lindex [split $tgt :] 0]] {
			lappend lna [list $lts [sql <- $tgt]]
		}
	}
#	puts $lts:$ltype:$lns:$lt:$lparams
}
mysqlclose $db

set slna [lsort -index 1 $lna]
foreach na $slna {
	puts $na
}
...
#puts [lsort $l1]
#puts [lsort -index 0 -integer $l2]

exit

mysqlreceive $db "
	select log_page, rev_page, page_title
	from logging join revision on rev_page = log_page join page on page_id = log_page
	where !log_namespace and rev_timestamp >= 20171219000000 and !page_namespace and page_is_new = 1
;" {lp rp pt} {
	puts $lp:$rp:$pt
}
...
mysqlreceive $db {
	select log_id, log_type, log_action, log_timestamp, log_user, log_user_text, log_namespace, log_title, log_page, log_comment, log_params, log_deleted
	from logging
	where log_timestamp >= 20171219000000 and !log_namespace and log_type not in ('review','patrol')
...
#puts bwwreviewmatch:$bwwreviewmatch
			if ![empty bwwreviewmatch] {
				foreach bwwreview [lsort -unique $bwwreviewmatch] {lappend orpt '[sql <- $bwwreview]'}
				mysqlreceive $db "
					select page_title
					from page join templatelinks on tl_from = page_id
					where page_title in ([join $orpt ,]) and !page_namespace and !tl_from_namespace and tl_namespace = 10 and tl_title = 'Review'
					order by page_title
				;" pt {
					lappend bwwreviewblock "\[\[:[sql -> $pt]\]\]"
				}
				unset -nocomplain orpt
			}
