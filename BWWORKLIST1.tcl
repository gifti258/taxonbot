...
if {[exec pgrep -cxu taxonbot BWWORKLIST1.tcl] > 1} {exit}

source api2.tcl
set lang kat ; source langwiki.tcl
source procs.tcl

set lportal [cat {Kategorie:Wikipedia:MerlBot-Listen Typ (BWWORKLIST)} x]
foreach portal $lportal {lappend clportal [string map {{ } _ ~ ~~~~~ / ~ ! ´´´´} $portal]}
foreach portal [glob -directory BWWORKLIST -tails *] {
	if {$portal ni $clportal && $portal ni [glob -directory BWWORKLIST -tails @*]} {
		puts "rm: BWWORKLIST/$portal"
		exec rm BWWORKLIST/$portal
	}
}
...
foreach portal $lportal {
	...
	regexp -- {\{\{(Benutzer:MerlBot/InAction\|BWWORKLIST\|.*?)\}\}} $contents -- temp
   ...
   set listform [dict values [regexp -inline -- {(SHORTLIST|CLIST|TABLE)} $lparam]]
	...
	if {{KALP} ni [dict keys $lparam]} {lappend lparam KALP {}}
	dict with lparam {
		...
		set f [open BWWORKLIST/[string map {{ } _ ~ ~~~~~ / ~ ! ´´´´} $portal] w] ; puts $f $lxcat ; close $f
	}
}
