...
proc contents {param item section} {
	...
	if {$section eq {x}} {
		set contents [encoding convertfrom [encoding convertfrom [encoding convertto [content [post $wiki {*}$get / [expr {$param eq {id}?{pageids}:$param eq {t}?{titles}:{}}] $item / utf8]]]]]
	} else {
		set contents [encoding convertfrom [encoding convertfrom [encoding convertto [content [post $wiki {*}$get / [expr {$param eq {id}?{pageids}:$param eq {t}?{titles}:{}}] $item / rvsection $section / utf8]]]]]
	}
}
...
proc conts {param item section} {
	global wiki get
	if {$section eq {x}} {
		return [encoding convertfrom [encoding convertfrom [encoding convertto [content [post $wiki {*}$get / [expr {$param eq {id}?{pageids}:$param eq {t}?{titles}:{}}] $item / utf8]]]]]
	} else {
		return [encoding convertfrom [encoding convertfrom [encoding convertto [content [post $wiki {*}$get / [expr {$param eq {id}?{pageids}:$param eq {t}?{titles}:{}}] $item / rvsection $section / utf8]]]]]
	}
}
...
proc conts2 {param item section} {
	global wiki get
	if {$section eq {x}} {
		return [content [post2 $wiki {*}$get / [expr {$param eq {id}?{pageids}:$param eq {t}?{titles}:{}}] $item / utf8]]
	} else {
		return [content [post2 $wiki {*}$get / [expr {$param eq {id}?{pageids}:$param eq {t}?{titles}:{}}] $item / rvsection $section / utf8]]
	}
}
...
proc contentsx {param item section} {
	global contents wiki get
	set contents [content [post $wiki {*}$get / [expr {$param eq {id}?{pageids}:$param eq {t}?{titles}:{}}] $item / rvsection $section]]
}
...
proc d_get_lq {ent p} {
	try {
		switch [d_get_datatype $p] {
			...
			time				{
									foreach mainsnak [d_get_lmainsnak $ent $p] {
										lappend lq [lindex [split [
											dict get $mainsnak mainsnak datavalue value time
										] T] 0]
									}
								}
			wikibase-item	{
	...
	switch $datatype {
		...
		time				{
								...
								return [format {
									"datavalue":{
										"value":{
											"time":"%sT00:00:00Z","precision":11,%s,%s
										},"type":"time"
									},"datatype":"%s"
								} $q $par $cal $datatype]
							}
...
#		set res [lindex [join [lindex [[[dom parse -html $html] documentElement] asList] 2]] 5]
	}] {break} else {puts "$page: Items: SPARQL Fehler" ; puts [edit user:TaxonBot/Test {} $htmltest]}}
	return $res
}
