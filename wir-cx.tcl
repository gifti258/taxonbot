	...
	foreach {-- -- line} $ldata {
		...
		dict with drow {
			...
			if {![empty P18] && ![dict exists $dP18 $item]} {
				set P18 "\[\[Datei:[lindex [split [urldecode [lindex [join [lindex $P18 2]] 1]] /] end]|center|120px\]\]"
			...
			if {![empty P19Label] && [lindex $P19Label 1] ne {}} {
				set P19Label \[\[[lindex [join [lindex $P19Label 2]] 1]\]\]
			...
			if {![empty P20Label] && [lindex $P20Label 1] ne {}} {
				set P20Label \[\[[lindex [join [lindex $P20Label 2]] 1]\]\]
...
set th "<noinclude>\{| class=\"wikitable sortable\" style=\"width: 100%;\"\n! lfd. !! Wikis !! Name !! Beschreibung / Tätigkeit !! Bild !! Staats-<br />angehörigkeit !! Geburtsdatum !! Geburtsort !! Sterbedatum !! Sterbeort !! Wikidata-<br />Objekt"
if {$i >= 1000} {
	set ncount \{\{0|.\}\}[tdot $i]
} elseif {$i >= 100} {
	set ncount \{\{0|00\}\}$i
...
if {$mc >= 10000} {
   ...
   append ncount /\{\{0|00000\}\}$mc
}
set tf "|\}</noinclude><includeonly>$ncount</includeonly>"
