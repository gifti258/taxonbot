...
catch {if {[exec pgrep -cxu taxonbot bwwkat.tcl] > 1} {exit}}
...
lassign {} bwwKALPkandkat bwwKALPlwkandkat bwwKALPlwabwkat bwwreviewkat
...
mysqlreceive $db "
	...
	where tl_from = page_id and cl_from = page_id and !page_namespace and !tl_from_namespace and tl_namespace = 10 and (tl_title in ('Kandidat','Review','Schreibwettbewerb') or tl_title like 'Lesenswert-%')
	order by page_title
;" {pt ct} {
	dict lappend bwwKALPkat [sql -> $pt] Kategorie:[sql -> $ct]
}
puts "[clock format [clock seconds] -format %T]:bwwKALPkat komplett"

lappend bwwkat bwwKALPkat $bwwKALPkat

set f [open BWWORKLIST/@bwwkat.db w] ; puts $f $bwwkat ; close $f
...
mysqlreceive $db "
	select page_title, cl_to
	from page, templatelinks, categorylinks
	where tl_from = page_id and cl_from = page_id and !page_namespace and !tl_from_namespace and tl_namespace = 10 and tl_title = 'Lesenswert-Abwahl'
	order by page_title
;" {pt ct} {
	dict lappend bwwKALPlwabwkat [sql -> $pt] Kategorie:[sql -> $ct]
}
mysqlclose $db
puts "[clock format [clock seconds] -format %T]:bwwKALPlwabwkat komplett"

set db [get_db dewiki]
mysqlreceive $db "
	select page_title, cl_to
	from page, templatelinks, categorylinks
	where tl_from = page_id and cl_from = page_id and !page_namespace and !tl_from_namespace and tl_namespace = 10 and tl_title = 'Lesenswert-Kandidat'
	order by page_title
;" {pt ct} {
	dict lappend bwwKALPlwkandkat [sql -> $pt] Kategorie:[sql -> $ct]
}
mysqlclose $db
puts "[clock format [clock seconds] -format %T]:bwwKALPlwkandkat komplett"

set db [get_db dewiki]
mysqlreceive $db "
	select page_title, cl_to
	from page, templatelinks, categorylinks
	where tl_from = page_id and cl_from = page_id and !page_namespace and !tl_from_namespace and tl_namespace = 10 and tl_title = 'Review'
	order by page_title
;" {pt ct} {
	dict lappend bwwreviewkat [sql -> $pt] Kategorie:[sql -> $ct]
}
mysqlclose $db
puts "[clock format [clock seconds] -format %T]:bwwreviewkat komplett"

lappend bwwkat bwwKALPkandkat $bwwKALPkandkat bwwKALPlwabwkat $bwwKALPlwabwkat bwwKALPlwkandkat $bwwKALPlwkandkat bwwreviewkat $bwwreviewkat
...
mysqlreceive $db "
	select page_title, cl_to
	from page, templatelinks, categorylinks
	where tl_from = page_id and cl_from = page_id and !page_namespace and !tl_from_namespace and tl_namespace = 10 and (tl_title = 'Kandidat' or tl_title like 'Lesenswert-%' or tl_title = 'Review')
	order by page_title
;" {pt ct} {
	dict lappend bwwKALPkat [sql -> $pt] Kategorie:[sql -> $ct]
}
...
mysqlreceive $db "
	select page_title
	from $db0.page, $db0.categorylinks
...
foreach lparam $llparam {
	   ...
	   if {[string first { (CE} [lindex $lparam end]] > -1 && [lindex $lparam end-1] != 1} {set lparam [linsert $lparam end-1 1]}
		...
		mysqlreceive $db "
			select cl_to
			from $db0.categorylinks, $db0.page
