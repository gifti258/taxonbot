...
catch {if {[exec pgrep -cxu taxonbot autoarchiv-voy.] > 1} {exit}}

package require math::roman
package require unicode

source api.tcl ; set lang voy ; source langwiki.tcl ; #set token [login $wiki]
if {$cal(hour) in {23 11}} {
	set fl [open aa-voy.out w+]
	close $fl
	puts ...\n0:0
	exit
}
