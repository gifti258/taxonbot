...
set gettoken "$query / meta tokens"; # / type edit
...
set parse "$format / action parse"
set flagged "$query / prop revisions / rvprop flagged|ids|comment|user|timestamp"
set logevents "$query / list logevents"
set catmem "$query / list categorymembers / cmlimit max / cmprop title|sortkey"; # / cmtitle (Category:)~
...
set usercontribs "$query / list usercontribs / uclimit 1 / ucprop timestamp"; # ucuser ~
set blocked "$query / list users / usprop blockinfo"; # ususers ~
set redirect "$query / prop info / redirects true"; # titles ~
set backlinks "$query / list backlinks"
